...
module.exports = class BlockQueue {
  constructor (nanoeth, opts) {
    ...
    this.pollTime = opts.pollTime || 10000
    this.active = 0
    this.blockHeight = 0
    ...
    this.blockNumber = opts.blockNumber || (() => this.eth.blockNumber())
  }
  ...
  async confirmed () {
    while ((this.blockHeight - this.head) < this.confirmations && !this.destroyed) {
      this.blockHeight = Number(await this.blockNumber())
      if ((this.blockHeight - this.head) < this.confirmations) {
        await this.blocking.wait(this.pollTime)
        continue
      }
    }
  }
