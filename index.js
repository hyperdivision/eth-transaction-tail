    ...
    for (let i = 0; i < block.transactions.length; i++) {
      ...
      while (l < logs.length && Number(logs[l].transactionIndex) === i) {
        ...
        if (e.name === 'DEPOSIT_FORWARDED') {
          if (!(await this._filterTo(addr, addr))) continue
        }
        ...
        queue.push({ receipt, status: false, tx, log, event: e, filter: true, txs: null })
      }
      ...
      if (!filter && !f && !t) continue

      if (!receipt) receipt = this.getTransactionReceipt(tx.hash)

      const txs = (f || t) ? [].concat(f || []).concat(t || []) : null
  ...
  async loop () {
    while (!this.queue.destroyed) {
      ...
      for (const { status, tx, log, event, filter, txs } of queue) {
        ...
        if (txs) {
          for (const t of txs) t(tx, confirmations, block)
        }
      }
